...
artworksData.forEach(artwork => {
    const imagesHtml = artwork.images.map(image => 
        '<div class="lg:col-span-' + image.span + '"><img src="static' + image.src + '" alt="' + image.alt + '" class="w-full h-full object-cover rounded-lg" loading="lazy" /></div>'
    ).join('');
    
    const descriptionHtml = artwork.description ? 
        '<div class="mt-6 text-lg font-light tracking-wide text-gray-700 leading-relaxed">' + artwork.description + '</div>' : '';
